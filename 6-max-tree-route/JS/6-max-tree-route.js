...
function stringifyTree(root) {
  return root.value === null ? "" : `${root.value}${root.left ? "\n" + stringifyTree(root.left) : ""}${root.right ? "\n" + stringifyTree(root.right) : ""}`;
}
...
const maxSum = (root) => root === null ? 0 
      : root.left  === null && root.right === null ? root.value
      : root.value + Math.max(
...
function main() {
  ...
  const sampleTree = new TreeNode(17, new TreeNode(3, new TreeNode(2)), new TreeNode(-10, new TreeNode(16), new TreeNode(1, new TreeNode(13))));
